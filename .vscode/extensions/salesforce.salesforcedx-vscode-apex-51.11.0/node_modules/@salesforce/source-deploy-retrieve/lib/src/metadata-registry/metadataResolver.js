...
class MetadataResolver {
    ...
    getComponentsFromPathRecursive(dir) {
        const dirQueue = [];
        const components = [];
    ...
    resolveType(fsPath) {
        ...
        for (const type of this.registry.getStrictFolderTypes()) {
            if (pathParts.has(type.directoryName)) {
                ...
                if (!type.inFolder || path_2.parentName(fsPath) !== type.directoryName) {
                    resolvedType = type;
                }
                break;
            }
        ...
        if (!resolvedType) {
            resolvedType = this.registry.getTypeBySuffix(path_2.extName(fsPath));
        }
        return resolvedType;
    }
    /**
    ...
    parseAsContentMetadataXml(fsPath) {
        return !!this.registry.getTypeBySuffix(path_2.extName(fsPath));
    }
    /**
    ...
    isMetadata(fsPath) {
        return (!!registry_1.parseMetadataXml(fsPath) ||
            this.parseAsContentMetadataXml(fsPath) ||
