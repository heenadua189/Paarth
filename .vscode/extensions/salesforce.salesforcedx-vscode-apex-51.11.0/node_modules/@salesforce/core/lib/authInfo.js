...
exports.AuthInfo = exports.DEFAULT_CONNECTED_APP_INFO = exports.SfdcUrl = exports.OAuth2WithVerifier = void 0;
...
function getJwtAudienceUrl(options) {
    ...
    else if (createdOrgInstance.startsWith('cs') ||
        createdOrgInstance.endsWith('s') ||
        url_1.parse(loginUrl).hostname === 'test.salesforce.com') {
...
class AuthInfo extends kit_1.AsyncCreatable {
    ...
    async save(authData) {
        ...
        const config = await authInfoConfig_1.AuthInfoConfig.create(Object.assign(Object.assign({}, authInfoConfig_1.AuthInfoConfig.getOptions(username)), { throwOnNotFound: false }));
    ...
    async init() {
        ...
        if (this.options.username && this.options.oauth2Options) {
            const authInfoConfig = await authInfoConfig_1.AuthInfoConfig.create(Object.assign(Object.assign({}, authInfoConfig_1.AuthInfoConfig.getOptions(this.options.username)), { throwOnNotFound: false }));
    ...
    async loadAuthFromConfig(username) {
        if (AuthInfo.cache.has(username)) {
            ...
            try {
                const config = await authInfoConfig_1.AuthInfoConfig.create(Object.assign(Object.assign({}, authInfoConfig_1.AuthInfoConfig.getOptions(username)), { throwOnNotFound: true }));
    ...
    async buildRefreshTokenConfig(options) {
        ...
        if (!username) {
            ...
            // eslint-disable-next-line @typescript-eslint/ban-ts-ignore
            // @ts-ignore TODO: need better typings for jsforce
            authFieldsBuilder.instance_url, authFieldsBuilder.access_token, ts_types_1.ensureString(orgId), ts_types_1.ensureString(userId));
        }
        return {
            orgId,
            username,
    ...
    async exchangeToken(options, oauth2 = new jsforce_1.OAuth2(options)) {
        ...
        if (!username) {
            ...
            // eslint-disable-next-line @typescript-eslint/ban-ts-ignore
            // @ts-ignore TODO: need better typings for jsforce
            authFields.instance_url, authFields.access_token, ts_types_1.ensureString(orgId), ts_types_1.ensureString(userId));
        }
        return {
    ...
    async retrieveUsername(instanceUrl, accessToken, orgId, userId) {
        ...
        try {
            ...
            return ts_types_1.asString(kit_1.parseJsonMap(response.body).Username);
        }
        catch (err) {
